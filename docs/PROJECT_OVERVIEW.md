...
| [`list_open_orders.py`](../list_open_orders.py) | List your unfilled orders and currently-held (not-yet-resolved) positions; interactively cancel an order or sell a position. When selling, it shows the live order book before the price prompt, and entering `0` at that prompt refreshes the top-5 bids/asks and asks again. |
